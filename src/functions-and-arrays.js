...
// should return: 57 // mine returns 238 :(
...
function sum(arr) {
  ...
  for (let index = 0; index < arr.length; index++) {
    ...
    if (
      ...
      throw new Error("Error: not acceptable data type")
    }

    if (typeof element === "number" || typeof element === "boolean") {
      sumAll += element
    } else if (typeof element === "string" && !isNaN(element)) {
      sumAll += parseFloat(element)
    }
  }
  return sumAll
}
...
function averageWordLength(arr) {
  ...
  for (let index = 0; index < arr.length; index++) {
    const element = arr[index]
    elementSum += element.length
  }
  const average = elementSum / arr.length
